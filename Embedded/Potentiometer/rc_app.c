...
#include "tasks/rc_task_pot/rc_task_pot.h"
...
PRIVATE_FCT void rcapp_SetupHardware(void)
{
	//T_RCLL_UART_CONFIG ls_UARTConfig;
...
//	xTaskCreate(rctask_XBee, (const char *) "XBee task",
//			RC_STACKSIZE_TASK_XBEE, NULL, (tskIDLE_PRIORITY + 1UL),
//			&xHandleXBee);
	xTaskCreate(rctask_LEDs, (const char *) "LEDs task",
			RC_STACKSIZE_TASK_LEDS, NULL, (tskIDLE_PRIORITY + 1UL),
			&xHandleLEDs);
	xTaskCreate(rctask_pot, (const char *) "POT task",
			RC_STACKSIZE_TASK_POT, NULL, (tskIDLE_PRIORITY + 1UL),
			&xHandleLEDs);
